...
def attributes(req):
    return render(req, 'core/attributes.html')
...
def model_prediction(file=None, entry=None, year=None, opt=None):
    ...
    prediction.drop("Multi Layer Perceptron",inplace=True)
    #prediction.drop("LDA",inplace=True)
    #prediction.drop("Random Forest",inplace=True)
    #prediction.drop("Gradient Boosting",inplace=True)
    #prediction.drop("Support Vector Machine",inplace=True)
    prediction.replace(to_replace=0,value="NOT BANKRUPT",inplace=True)
    prediction.replace(to_replace=1,value="BANKRUPT",inplace=True)
